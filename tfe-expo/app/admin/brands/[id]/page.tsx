  ...
  useEffect(() => {
    async function fetchBrand() {
      try {
        ...
        setDocuments(docs.length ? docs : [{ title: "", description: "", access_type: "Download" }]);

        const vals = b.franchisee_validations as Validation[] ?? [];
        setValidations(vals.length ? vals : [{ initials: "", name: "", location: "", months_open: 0, quote: "", rating: 5 }]);
      } catch (err) {
    ...
    <div className="px-8 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.push(`/dashboard/pavilion/${slug}`)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">
            View pavilion →
          </button>
        ...
        {step === 0 && (
          <div className="space-y-5">
            ...
            <div>
              ...
              <textarea className={`${inputClass} h-28 resize-none`} value={description} onChange={(e) => setDescription(e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>Full overview (pavilion)</label>
              <textarea className={`${inputClass} h-36 resize-none`} value={overview} onChange={(e) => setOverview(e.target.value)} />
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="item19" className="text-sm text-slate-700">Item 19 present in FDD</label>
            </div>
            {item19Present && (
              <>
                <div className="grid grid-cols-2 gap-5">
                    ...
                    {pnlRows.map((row, i) => (
                      <div key={i} className="rounded-lg border border-slate-200 p-4">
                        <div className="mb-3">
                          ...
                          <input className={inputClass} value={row.label} onChange={(e) => { const u = [...pnlRows]; u[i] = { ...u[i], label: e.target.value }; setPnlRows(u); }} />
                        </div>
                ...
                {territories.map((t, i) => (
                  <div key={i} className="rounded-lg border border-slate-200 p-4">
                    <div className="mb-3">
                      ...
                      <input className={inputClass} value={t.name} onChange={(e) => { const u = [...territories]; u[i] = { ...u[i], name: e.target.value }; setTerritories(u); }} />
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="w-48">
                        <label className={labelClass}>Status</label>
                        <select className={inputClass} value={t.status} onChange={(e) => { const u = [...territories]; u[i] = { ...u[i], status: e.target.value as Territory["status"] }; setTerritories(u); }}>
                ...
                <div key={i} className="rounded-lg border border-slate-200 p-4">
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <select className={inputClass} value={doc.access_type} onChange={(e) => { const u = [...documents]; u[i] = { ...u[i], access_type: e.target.value as Document["access_type"] }; setDocuments(u); }}>
                        <option>Download</option><option>Request access</option>
                      </select>
                    </div>
